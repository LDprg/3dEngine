...
{
	auto args = static_cast<ApiThreadArgs*>(userData);
	...
	while (!exit)
	{
		...
		while (auto ev = static_cast<EventType*>(s_apiThreadEvents.pop()))
		{
			if (*ev == EventType::Key)
			{
		...
		bgfx::dbgTextImage(bx::max<uint16_t>(static_cast<uint16_t>(width / 2 / 8), 20) - 20,
		                   bx::max<uint16_t>(static_cast<uint16_t>(height / 2 / 16), 6) - 6, 40, 12, s_logo, 160);
