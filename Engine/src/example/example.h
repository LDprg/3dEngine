...
{
	const auto keyEvent = new KeyEvent;
	keyEvent->key = key;
...
{
	const auto args = static_cast<ApiThreadArgs*>(userData);
	...
	while (!exit)
	{
		...
		while (const auto ev = static_cast<EventType*>(s_apiThreadEvents.pop()))
		{
			if (*ev == EventType::Key)
			{
				const auto keyEvent = reinterpret_cast<KeyEvent*>(ev);
		...
		bgfx::dbgTextClear();
		bgfx::dbgTextImage(bx::max<uint16_t>(static_cast<uint16_t>(width / 2 / 8), 20) - 20,
		                   bx::max<uint16_t>(static_cast<uint16_t>(height / 2 / 16), 6) - 6, 40, 12, logo, 160);
...
int TEST()
{
	...
	apiThreadArgs.width = static_cast<uint32_t>(width);
	apiThreadArgs.height = static_cast<uint32_t>(height);
	bx::Thread apiThread;
